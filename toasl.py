...
bot = telebot.AsyncTeleBot("2011601337:AAFI0aayBsl6iSY2n4k-uHdWI9DyIyYuDmg")

@bot.message_handler(commands=['start'])
def start(message):
    if message.chat.type == 'private':
        id = message.from_user.id
        req = requests.get(f'https://api.telegram.org/bot2011601337:AAFI0aayBsl6iSY2n4k-uHdWI9DyIyYuDmg /getChatMember?chat_id=@rickpro3&user_id={id}', headers=headerss)
        ...
        if s == 'left':
            ...
            ⌔︙مطور البوت : @SSSGBS ''', reply_to_message_id=message.message_id)
...
def send(message):
    bot.forward_message("1496119874", message.chat.id,message.message_id)
